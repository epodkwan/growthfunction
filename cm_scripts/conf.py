...
class Configuration:
    ...
    #cosmo_dtype: jnp.dtype(jnp.float64)
    #pmid_dtype:  jnp.dtype(jnp.int16)
    #float_dtype: jnp.dtype(jnp.float32)

    cosmo_dtype: Optional[Any] = jnp.dtype(jnp.float64)
    pmid_dtype:  Optional[Any] = jnp.dtype(jnp.int16)
    float_dtype: Optional[Any] = jnp.dtype(jnp.float32)
    ...
    def growth_a(self):
        ...
        if self.growth_anum is not None:
            return jnp.linspace(0, 1., self.growth_anum)
        else:
