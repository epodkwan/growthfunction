...
# %%
def normalize(a):
    return growth(a)/growth(1)

# %%
x_data=torch.linspace(0.01,1,2000)
y_data=normalize(x_data)
data=torch.ones(2000,5)
data[:,0]=om_m
data[:,1]=om_
data[:,2]=om[2]
data[:,3]=h0
...
test_x=torch.linspace(0.01,1,600)
test[:,0]=test_x
true_y=normalize(test_x)
